...
This Evidence report replaces is built directly from BigQuery mart models.
...
select
  transport_type_label,
  round(sum(events_on_time) / nullif(sum(total_events), 0), 2) as on_time_pct,
  round(sum(total_cancelled) / nullif(sum(total_events), 0), 2) as cancelled_pct
from sbb_punctuality_marts.transport_daily
where operating_day between '${inputs.report_range.start}' and '${inputs.report_range.end}'
  and transport_type_label in ${inputs.transport_filter.value}
group by 1
